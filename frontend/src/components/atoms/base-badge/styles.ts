...
import { microLabel, popIn } from '@/styles'

export type BaseBadgeTone = 'neutral' | 'success' | 'pending' | 'danger'
...
export const BadgeChip = styled.span<BadgeProps>`
  ${microLabel};
  ...
  ${({ $isAnimated }) =>
    $isAnimated &&
    css`
      animation: ${popIn} ${({ theme }) => theme.transitions.spring} both;
    `}
`

/* Ponto de cor que antecede o rotulo, no lugar do antigo carimbo. */
